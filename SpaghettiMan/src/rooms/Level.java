...
public class Level{
	...
	/**
	 * This method is the heart and soul of the class. It generates every level procedurally.
	...
	public static Room[][] generateLevel(int stage) {
		...
		level[(level.length-1)/2][(level.length-1)/2] = new Room(new ArrayList<EnvironmentMember>(), generateDoors(level, (level.length-1)/2, (level.length-1)/2));
		...
		level[itemX][itemY] = new Room(new ArrayList<EnvironmentMember>()/*TODO: generate item room*/, generateDoors(level, itemX, itemY));
